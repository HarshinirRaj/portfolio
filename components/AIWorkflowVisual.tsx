const nodes = [
  { label: "engineer intent", sub: "prompt + spec" },
  { label: "context repo", sub: "LLM-readable architecture docs" },
  { label: "agent run", sub: "MCP tools · Claude Code" },
  { label: "generated change", sub: "code + tests" },
  { label: "shipped", sub: "reviewed, gates held" },
];
    ...
    >
      <div className="mb-5 flex items-center justify-between font-mono text-[11px] uppercase tracking-[0.12em] text-[rgb(var(--text-muted))]">
        <span className="flex items-center gap-2">
          <span className="h-1.5 w-1.5 rounded-full bg-signal-amber animate-blink" />
          ai workflow
        </span>
        <span>30%+ faster</span>
      </div>
        ...
        {nodes.map((node, i) => (
          <div key={node.label}>
            <div className="flex items-center gap-3 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--surface-2))] px-3.5 py-2.5">
              ...
              {i === nodes.length - 1 && (
                <span className="ml-auto shrink-0 rounded-full bg-signal-amber/15 px-2 py-0.5 font-mono text-[10px] text-signal-amber">
                  ✓
                </span>
              )}
            </div>
            ...
            {i < nodes.length - 1 && (
              <div className="relative ml-6 h-6 w-px bg-[rgb(var(--border))]">
                <span className="absolute left-1/2 top-0 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-signal-cyan animate-drop" />
              </div>
            )}
          </div>
        ))}
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <div className="rounded-lg border border-[rgb(var(--border))] px-3 py-2.5">
          <p className="font-mono text-lg text-signal-amber font-tabular">30%+</p>
          <p className="font-mono text-[10px] uppercase tracking-[0.08em] text-[rgb(var(--text-muted))]">
            faster delivery
          </p>
        </div>
        <div className="rounded-lg border border-[rgb(var(--border))] px-3 py-2.5">
          <p className="font-mono text-lg text-signal-cyan font-tabular">0</p>
          <p className="font-mono text-[10px] uppercase tracking-[0.08em] text-[rgb(var(--text-muted))]">
            gates loosened
          </p>
        </div>
